...
program returns [Program p]
        ...
        @after{p = new Program(funcList);}
        :
        (function{funcList.add($function.f);})+ EOF
	;
...
formalParameterList returns [List<FormalParameter> fpl]
        @init{List<FormalParameter> list = new ArrayList<FormalParameter>();}
        @after{fpl = list;}
        :
        fp1 = formalParameter{list.add($fp1.fp);} (COMMA fpi = formalParameter{list.add($fpi.fp);})*
        |
        ;
